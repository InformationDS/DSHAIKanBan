...
`)
    ...
    async function refresh() {
      try {
        const r = await host.call('getState')
    ...
    function HandoffPanel(props) {
      ...
      async function generate() {
        const r = await host.call('handoffContext', { workItemId: props.workItemId, note: note })
    ...
    function convCtxStatus(st, conv, workItemId) {
      ...
      if ((pm ? pm.id : null) === rec.projectMemory && (tm ? tm.id : null) === rec.taskMemory) return '最新'
      return '已过期'
    }
    ...
    function Board(props) {
      ...
      const columns = STATUSES.map((s) => {
        ...
        const cards = items.map((w) => {
          const convCount = st.conversations.filter((c) => c.workItems.indexOf(w.id) >= 0).length
    ...
    function ItemDetail(props) {
      ...
      const convs = st.conversations.filter((c) => c.workItems.indexOf(wi.id) >= 0)
      ...
      const convsPanel = h('div', { className: 'kb-panel', style: { marginTop: 12 } },
        h('h4', null, '参与本任务的对话（' + convs.length + '）'),
        convs.length === 0 ? h('div', { className: 'kb-muted' }, '还没有对话参与。在聊天中让 agent 起草建议或生成交接上下文后，本会话会自动登记进来。') : null,
        convs.slice().reverse().map((c) => {
          ...
          return h('div', { key: c.id, className: 'kb-conv', onClick: () => props.setNav({ kind: 'conv', id: c.id }) },
            ...
            h('div', { className: 'kb-row' },
              ...
              h('span', { className: 'kb-chip ' + (ctx === '最新' ? 'latest' : ctx === '已过期' ? 'expired' : '') }, '上下文 ' + ctx),
      ...
      const tasksPanel = h('div', { className: 'kb-panel' },
        ...
        c.workItems.map((wid) => {
          ...
          return h('div', { key: wid, className: 'kb-conv', onClick: () => props.setNav({ kind: 'item', id: wid }) },
            h('div', { className: 'kb-conv-title' }, wi.title),
            h('div', { className: 'kb-row' },
              h('span', { className: 'kb-chip ' + (STATUS_CLASS[wi.status] || '') }, wi.status),
              h('span', { className: 'kb-chip ' + (ctx === '最新' ? 'latest' : ctx === '已过期' ? 'expired' : '') }, '上下文 ' + ctx)))
        }))
