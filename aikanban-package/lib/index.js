export const name = 'dsh-aikanban'

const plugin = {
  inject: ['fs', 'tools', 'sandboxPolicy', 'sessions', 'agents', 'sessionPersistence', 'workspaceRegistry', 'webServer'],
  apply(ctx) {
    ...
    function fallbackTitle(id) {
      return '会话 ' + String(id || 'unknown').slice(0, 8)
    }

    // 载入后执行：conversation 加法式扩展 + 迁移。workItemId 缺失 → 取 workItems[0]（最早登记归属）。
    // title/live 为派生字段，不落盘：清除旧文件里可能残留的 title/live 副本，避免双源。
    function normalizeConversations() {
      ...
      for (const c of state.conversations) {
        ...
        if (typeof c.workItemId !== 'string' || !c.workItemId) {
          c.workItemId = c.workItems.length ? c.workItems[0] : ''
        } else if (c.workItems.indexOf(c.workItemId) === -1) {
          c.workItems.unshift(c.workItemId)
        }
        if (Object.prototype.hasOwnProperty.call(c, 'title')) delete c.title
        if (Object.prototype.hasOwnProperty.call(c, 'live')) delete c.live
        if (typeof c.createdAt !== 'string') c.createdAt = nowIso()
        if (typeof c.lastActiveAt !== 'string') c.lastActiveAt = nowIso()
      }
    }

    // 解析会话真实标题：活会话 sessionTitle.get；非活会话 sessionQuery.readTitleSnapshots 批量读；
    // 两个服务均可选，缺失时整体回退占位标题。
    async function resolveTitles(ids) {
      ...
      const sess = ctx.get('sessions')
      const seen = {}
      for (const id of ids || []) {
        if (!id || seen[id]) continue
        seen[id] = true
        const s = sess ? sess.get(id) : undefined
        if (s) liveIds.push({ id, s })
        else rest.push(id)
      }
      for (const { id, s } of liveIds) {
        try {
          if (st) {
            const snap = st.get(s)
            if (snap && typeof snap.title === 'string' && snap.title) out[id] = snap.title
          }
        } catch (err) {}
        if (!out[id]) out[id] = fallbackTitle(id)
      }
      if (rest.length && sq && typeof sq.readTitleSnapshots === 'function') {
        try {
          const results = await sq.readTitleSnapshots(rest)
          for (const r of results || []) {
            if (!r || typeof r !== 'object') continue
            const rid = r.sessionId
            if (r.status === 'fulfilled' && r.value && r.value.title && typeof r.value.title.title === 'string' && r.value.title.title) {
              out[rid] = r.value.title.title
            }
          }
        } catch (err) {}
      }
      for (const id of rest) if (!out[id]) out[id] = fallbackTitle(id)
      return out
    }
    ...
    // v3 绑定语义：新对话首次登记即绑定该工作项；已绑定 A 再参与 B → 只记 workItems 历史 + contextVersions，不改绑。
    function recordConversation(exec, workItemId) {
      ...
      if (!conv) {
        conv = { id: sid, workItemId: workItemId || '', workItems: [], contextVersions: {}, createdAt: nowIso(), lastActiveAt: nowIso() }
        state.conversations.push(conv)
      }
      if (workItemId) {
        if (conv.workItems.indexOf(workItemId) === -1) conv.workItems.push(workItemId)
        if (!conv.workItemId) conv.workItemId = workItemId
    ...
    async function opDeleteWorkItem(args, exec) {
      ...
      for (const c of state.conversations) {
        if (c.workItemId === item.id) c.workItemId = ''
    ...
    function rpc(method, fn) {
      handlers[method] = async (args, exec) => {
        try {
          const r = await fn(args, exec)
          // 统一把返回值里的 state 替换为带派生 title/live 的视图，UI 拿到一致数据
          if (r && r.ok === true && r.state !== undefined) {
            r.state = await viewState(exec)
          }
          return cleanJson(r)
        } catch (err) {
    ...
    // 绑定：会话已有归属 A 且未带 confirm → needsConfirm；confirm 或未绑定 → 解绑 A、绑 target、补历史与 contextVersions。
    async function opBindConversation(args, exec) {
      await ensureReady()
      await refine(exec)
      const workItemId = strOr(args && args.workItemId)
      const wi = findWorkItem(workItemId)
      ...
      if (!sid || sid === 'unknown') return { ok: false, error: '无法确定会话' }
      let conv = state.conversations.find((c) => c.id === sid)
      if (!conv) {
        conv = { id: sid, workItemId: '', workItems: [], contextVersions: {}, createdAt: nowIso(), lastActiveAt: nowIso() }
        state.conversations.push(conv)
      }
      const from = conv.workItemId && conv.workItemId !== workItemId ? conv.workItemId : ''
      if (from && !(args && args.confirm === true)) {
        const fromWi = findWorkItem(from)
        return { ok: false, needsConfirm: true, from, fromTitle: fromWi ? fromWi.title : '' }
      }
      if (from) conv.workItemId = ''
      if (conv.workItems.indexOf(workItemId) === -1) conv.workItems.push(workItemId)
      conv.workItemId = workItemId
      const pm = latestMemory('project', undefined)
      const tm = latestMemory('task', workItemId)
      conv.contextVersions[workItemId] = { projectMemory: pm ? pm.id : null, taskMemory: tm ? tm.id : null }
      conv.lastActiveAt = nowIso()
    ...
    // 解绑只清 workItemId：workItems / contextVersions / 建议 / 记忆版本来源均不动。
    async function opUnbindConversation(args, exec) {
      await ensureReady()
      await refine(exec)
      const sid = strOr(args && args.sessionId) || currentSessionId(exec)
      if (!sid || sid === 'unknown') return { ok: false, error: '无法确定会话' }
      const conv = state.conversations.find((c) => c.id === sid)
      if (!conv) return { ok: false, error: '对话不存在' }
      const got = strOr(args && args.workItemId)
      if (got && conv.workItemId && conv.workItemId !== got) return { ok: false, error: '对话当前绑定与 workItemId 不一致' }
      conv.workItemId = ''
      conv.lastActiveAt = nowIso()
      commit()
      return { ok: true, state }
    ...
    // 重命名仅活会话：sessionTitle.rename 同步 DSH 标题。
    async function opRenameConversation(args, exec) {
      await ensureReady()
      await refine(exec)
      const sid = strOr(args && args.sessionId) || currentSessionId(exec)
      const title = strOr(args && args.title).trim()
      ...
      const sess = ctx.get('sessions')
      const s = sess ? sess.get(sid) : undefined
      if (!s) return { ok: false, error: '仅已打开的会话可重命名' }
      const st = ctx.get('sessionTitle')
      if (!st) return { ok: false, error: 'sessionTitle 服务不可用，重命名已禁用' }
      try {
        const snap = st.rename(s, title)
        return { ok: true, title: snap && typeof snap.title === 'string' ? snap.title : title }
      } catch (err) {
        return { ok: false, error: String((err && err.message) || err) }
      }
    }

    // 会话候选列表：合并活会话 + 持久化会话，按规范化 cwd 过滤到当前工作区；过滤失败降级全量（标注工作区）。
    async function opListSessions(args, exec) {
      await ensureReady()
      await refine(exec)
      const map = {}
      const sess = ctx.get('sessions')
      const live = sess ? sess.list() : []
      for (const s of live) {
        if (!s || typeof s.id !== 'string' || !s.id) continue
        const h = s.header || {}
        map[s.id] = { id: s.id, live: true, cwd: typeof h.cwd === 'string' ? h.cwd : '' }
      }
      try {
        const persist = ctx.get('sessionPersistence')
        const headers = persist ? await persist.list() : []
        for (const h of headers || []) {
          if (!h || typeof h.id !== 'string' || !h.id) continue
          if (!map[h.id]) map[h.id] = { id: h.id, live: false, cwd: typeof h.cwd === 'string' ? h.cwd : '' }
          else if (!map[h.id].cwd && typeof h.cwd === 'string') map[h.id].cwd = h.cwd
        }
      } catch (err) {}
      let entries = Object.keys(map).map((id) => map[id])
      const ws = normPath(state.workspace)
      if (ws) {
        const filtered = entries.filter((e) => e.cwd && normPath(e.cwd) === ws)
        if (filtered.length) entries = filtered
      }
      const titles = await resolveTitles(entries.map((e) => e.id))
      return {
        ok: true,
        workspace: state.workspace,
        sessions: entries.map((e) => {
          const conv = state.conversations.find((c) => c.id === e.id)
          const boundId = conv && conv.workItemId ? conv.workItemId : ''
          const boundWi = boundId ? findWorkItem(boundId) : undefined
          return {
            id: e.id,
            title: titles[e.id] || fallbackTitle(e.id),
            live: e.live,
            boundWorkItemId: boundId || null,
            boundWorkItemTitle: boundWi ? boundWi.title : null,
            workspace: e.cwd || null,
            lastActiveAt: conv ? conv.lastActiveAt : null,
          }
        }),
      }
    }
    ...
    rpc('bindConversation', opBindConversation)
    ...
    // 工具注册需兼容两种运行环境：
    // 1) host 组合插件（嵌入包经 cordis.patch.yml 加载）：没有全局 harness，直接用 ctx.tools.register；
    // 2) 动态插件 sandbox（cordis_define 重建）：Guard 只接受 harness.defineTool 打标的工具。
    // 用 typeof 探测，避免在组合环境触发 ReferenceError: harness is not defined。
    function tool(name, description, schema, fn) {
      ...
      if (typeof harness !== 'undefined' && harness && typeof harness.defineTool === 'function') {
        const def = harness.defineTool(definition)
    ...
    tool('kanban_view', '查看 AIKanBan 看板当前状态：工作项列表、任务/项目记忆版本数、参与对话、活动建议与待审核建议。', { properties: {} }, async (args, exec) => {
      ...
      const titles = await resolveTitles(convIds)
      const sess = ctx.get('sessions')
      return {
        workspace: state.workspace,
        storage: storageDiag,
        ...
        conversations: state.conversations.map((c) => ({ id: c.id, title: titles[c.id] || fallbackTitle(c.id), live: !!(sess && sess.get(c.id)), workItemId: c.workItemId, workItems: c.workItems, lastActiveAt: c.lastActiveAt })),
    ...
    tool('kanban_bind_conversation', '把当前会话绑定到指定工作项。一个工作项可绑定多条对话；一条对话同一时间只绑定一个工作项。若当前会话已绑定其他工作项，须用户明确同意改绑后带 confirm:true 再次调用；返回 needsConfirm 时应向用户说明并等待同意。', {
      properties: {
        workItemId: { type: 'string', description: '要绑定的工作项 id（必填）' },
        confirm: { type: 'boolean', description: '改绑确认：当前会话已绑定其他工作项时，用户明确同意后传 true（选填，默认 false）' },
      },
      required: ['workItemId'],
    }, opBindConversation)

    tool('kanban_unbind_conversation', '解绑当前会话的绑定关系。只清绑定，不动该对话的参与历史、上下文版本记录、产出的建议与记忆版本。', {
      properties: {
        workItemId: { type: 'string', description: '可选；传入时校验当前绑定与该工作项一致才解绑' },
      },
      required: [],
    }, opUnbindConversation)

    // Persistent profile bundle API for the browser client.
    const HTTP_PREFIX = '/aikanban-api'
    ...
    async function handleApi(req, res) {
      ...
      const exec = typeof args.sessionId === 'string' && args.sessionId ? { agent: { id: args.sessionId } } : undefined
      const handler = handlers[method]
      if (!handler) return sendJson(res, 404, { ok: false, error: 'unknown method: ' + method })
      try {
        const result = await handler(args, exec)
        return sendJson(res, 200, result)
      } catch (err) {
        console.error('aikanban api', method, err)
